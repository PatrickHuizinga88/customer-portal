...
const formSchemaPassword = toTypedSchema(z.object({
  current_password: z.string({message: t('common.validations.required')}),
  new_password: z.string().min(8, {
    message: t('authentication.validations.password_length', {length: 8})
  }),
...
<template>
  <dl class="grid sm:grid-cols-2 gap-8">
    <div>
      <div class="space-y-2">
        <DescriptionItem :label="$t('profile.email')" :value="email"/>
          ...
          <Dialog v-model:open="emailDialogOpen">
            <DialogTrigger as-child>
              <Button variant="outline" class="w-full sm:w-auto">
                <Pencil/>
                {{ capitalizeSentence($t('common.actions.edit_item', {item: $t('profile.email')})) }}
              </Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
    ...
    <div>
      <DescriptionItem :label="$t('authentication.common.password')" value="··············"/>
